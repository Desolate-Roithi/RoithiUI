...
local function ProtectFontObjects(selection)
    ...
    for _, lbl in ipairs(labels) do
        if lbl and lbl.ApplyFontObjects then
            lbl.ApplyFontObjects = function(fontString)
                if not fontString or not fontString.fontObjectsToTry then return end
                for _, fontObj in ipairs(fontString.fontObjectsToTry) do
                    ...
                    local isSec = (issecretvalue and issecretvalue(isTrunc)) or (type(isTrunc) == "userdata")
                    if not ok or isSec or not isTrunc then
                        break
                    end
                end
            end
...
local function ProtectSelectionTooltip(selection)
...
local function ProtectGetPoint(frameObj, defaultObj)
    if not frameObj or not frameObj.GetPoint or frameObj.roithiGetPointSecured then return end
...
local function ProtectFrameBounds(frameObj)
    if not frameObj or frameObj.roithiBoundsSecured then return end
        ...
        if type(frameObj[method]) == "function" then
            ...
            frameObj[method] = function(s, ...)
                ...
                for _, v in ipairs(results) do
                    if (issecretvalue and issecretvalue(v)) or (type(v) == "userdata") then
                        isSec = true
                        break
                    end
                end
                ...
                if isSec then
                    ...
                    local isFwSec = (issecretvalue and issecretvalue(rawFw)) or type(rawFw) == "userdata" or not rawFw
                    local isFhSec = (issecretvalue and issecretvalue(rawFh)) or type(rawFh) == "userdata" or not rawFh
...
function RoithiUI:OnInitialize()
    ...
    self.db.RegisterCallback(self, "OnNewProfile", "RefreshProfile")
    self.db.RegisterCallback(self, "OnProfileDeleted", "RefreshProfile")
    ...
    if LEM and LEM.AddFrame and not LEM.roithiFontSecured then
        ...
        LEM.AddFrame = function(selfObj, frameObj, callbackObj, defaultObj, nameObj)
            ...
            if selection then
                ProtectFontObjects(selection)
                ProtectSelectionTooltip(selection)

                -- Elevate selection overlay for Aura Movers so they are always clickable over overlapping unit frames (below popup dialogs)
                if frameObj.containerRef or frameObj.containerSuffix then
                    pcall(function()
                        frameObj:SetFrameStrata("DIALOG")
                        frameObj:SetFrameLevel(200)
                        selection:SetFrameStrata("DIALOG")
                        selection:SetFrameLevel(500)
                    end)
                end

                -- FIX: If AddFrame is called while Edit Mode is ALREADY ACTIVE, show the selection frame!
                if selfObj.isEditing or (EditModeManagerFrame and EditModeManagerFrame:IsShown()) then
                    selection:Show()
                end
            end

            ProtectGetPoint(frameObj, defaultObj)
            ProtectFrameBounds(frameObj)
        end
    end
...
function RoithiUI:RefreshProfile()
    ...
    local UF = self:GetModule("UnitFrames", true)
    if UF then
        if UF.units then
            for unit in pairs(UF.units) do
                ...
                if ns.AttachmentLogic then
                    ns.AttachmentLogic:GlobalLayoutRefresh(unit)
                end
            end
        end
        if UF.UpdateAllCustomAuras then
            UF:UpdateAllCustomAuras()
        end
        if UF.RefreshBossEditMode then
            UF:RefreshBossEditMode()
        end
    end

    local EB = self:GetModule("EncounterBar", true)
    if EB and EB.OnEnable then
        EB:OnEnable()
    end

    -- Refresh AceConfigRegistry to match new DB
    local ACR = LibStub("AceConfigRegistry-3.0", true)
    if ACR then
        ACR:NotifyChange("RoithiUI")
        ACR:NotifyChange("RoithiUI_Profiles")
    end
