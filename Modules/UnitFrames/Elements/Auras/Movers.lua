...
local function GetOrCreateAuraMover(container, unit, containerSuffix)
    ...
    if mover.SetFrameStrata then mover:SetFrameStrata("DIALOG") end
    if mover.SetFrameLevel then mover:SetFrameLevel(200) end
    if mover.SetClampedToScreen then mover:SetClampedToScreen(true) end
    if mover.SetMovable then mover:SetMovable(true) end
    if mover.EnableMouse then mover:EnableMouse(false) end   -- default click-through; toggled per mode
    mover:Hide()
    ...
    if sampleGroup.SetFrameLevel then
        sampleGroup:SetFrameLevel(((mover.GetFrameLevel and mover:GetFrameLevel()) or 200) + 1)
    end
    if sampleGroup.EnableMouse then
